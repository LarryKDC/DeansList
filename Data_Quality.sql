...
JOIN CUSTOM_DLATTENDANCE_RAW A ON A.BEHAVIORDATE = CD.DATE_VALUE AND I.STUDENTSCHOOLID = A.STUDENTSCHOOLID AND BEHAVIORCATEGORY = 'Daily Attendance' AND A.DLSCHOOLID = I.SCHOOLID --INCLUDE SCHOOLID BECAUSE LC GET AN ATTENDANCE RECORD AT THE HOMESCHOOL AND LC
JOIN POWERSCHOOL.POWERSCHOOL_STUDENTS S ON S.STUDENT_NUMBER = I.STUDENTSCHOOLID
...
------------Incorrect Attendance Code (X) 
select 
s.student_number as student_number, ----students
sch.abbreviation as school_name, -----schools
ac.att_code as errorgroup, ----attendance code
CAST(a.ATT_DATE as DATE) as error_date, -----attendance
cal.yearid as yearid, -----calendar
e.grade_level as grade_level, ----students
s.lastfirst as lastfirst, ----students 
'https://powerschool.kippdc.org/admin/students/allenrollments.html?frn=001'as link,
CASE 
  WHEN ((a.att_date > '08-AUG-16' AND e.grade_level BETWEEN -1 AND 8)
     OR (a.att_date > '15-AUG-16' AND (e.grade_level = -2 OR e.grade_level = 9))
     OR (a.att_date > '22-AUG-16' AND e.grade_level BETWEEN 10 AND 12)) 
	 AND att_code = 'X' 
    THEN 'Incorrect Attendance Code'
    ELSE NULL
END as error,
'PowerSchool' as sourcesystem,  
35 as errorid
FROM POWERSCHOOL.POWERSCHOOL_SCHOOLS SCH
             JOIN (
               SELECT ID STUDENTID, SCHOOLID, GRADE_LEVEL, ENTRYDATE, ENTRYCODE, EXITDATE, EXITCODE FROM POWERSCHOOL.POWERSCHOOL_STUDENTS
               UNION ALL
               SELECT STUDENTID, SCHOOLID, GRADE_LEVEL, ENTRYDATE, ENTRYCODE, EXITDATE, EXITCODE FROM POWERSCHOOL.POWERSCHOOL_REENROLLMENTS
             ) E ON E.SCHOOLID = SCH.SCHOOL_NUMBER AND SCH.SCHOOL_NUMBER!=2001
             JOIN POWERSCHOOL.POWERSCHOOL_STUDENTS S ON S.ID = E.STUDENTID
             JOIN POWERSCHOOL.POWERSCHOOL_CALENDAR_DAY CD ON CD.SCHOOLID = E.SCHOOLID AND CD.DATE_VALUE BETWEEN E.ENTRYDATE AND E.EXITDATE-1 AND CD.INSESSION = 1
             LEFT OUTER JOIN POWERSCHOOL.POWERSCHOOL_ATTENDANCE A ON A.STUDENTID = E.STUDENTID AND A.ATT_DATE = CD.DATE_VALUE AND ATT_MODE_CODE = 'ATT_MODEDAILY'
             LEFT JOIN POWERSCHOOL.POWERSCHOOL_ATTENDANCE_CODE AC ON AC.ID = A.ATTENDANCE_CODEID
JOIN (SELECT DISTINCT
        ...
        ,DATE_VALUE
        FROM POWERSCHOOL.POWERSCHOOL_CALENDAR_DAY CD
        ) cal on cal.date_value = a.att_date
where att_code = 'X' 
AND
CASE
 WHEN ((a.att_date > '08-AUG-16' AND e.grade_level BETWEEN -1 AND 8)
